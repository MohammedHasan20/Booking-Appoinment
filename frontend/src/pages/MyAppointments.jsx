...
const cancelAppointment = async (appointmentId) => {
  try {
    const { data } = await axios.post(
      ...
      { appointmentId, userId: userData._id }, // ✅ send both
      { headers: { token } }
    );
          ...
          >
            ...
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {!item.cancelled && !item.payment && !item.isCompleted &&
               <button onClick={()=>appointmentRazorpay(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-blue-600 hover:text-white transition-all duration-300'>
                Pay Online
              </button>}
